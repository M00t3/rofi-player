...
selected_playlist=$(sed '/#.*/d;/^$/d' ./playlists_and_musics.csv | awk -F',' '{print $1}' | rofi -dmenu -i -lines 10 -width 30 -p "Select Music or Playlist: ")
link=$(sed '/#.*/d;/^$/d' ./playlists_and_musics.csv | grep "^$selected_playlist," | awk -F',' '{print $2}')
is_url_link=$(sed '/#.*/d;/^$/d' ./playlists_and_musics.csv | grep "$selected_playlist" | awk -F',' '{print $3}')
